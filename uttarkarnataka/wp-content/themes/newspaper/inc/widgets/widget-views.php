 ...
 * @since      1.0.0
 */
class Newspaper_Views_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-newspaper-views posts-thumbnail-widget',
			'description' => __( 'Display the most views posts with thumbnails.', 'newspaper' )
		);

		// Create the widget.
		parent::__construct(
			'newspaper-views',                                        // $this->id_base
			__( '&raquo; Most Views Posts Thumbnails', 'newspaper' ), // $this->name
					...
					echo '<li>';
						...
						echo '<div class="entry-info">';
							...
							if ( $instance['show_view'] ) :
								echo '<span class="entry-view">- ' .  number_format_i18n( $view ) . __( ' Views', 'newspaper' ) . '</span>';
							endif;
						echo '</div>';
					echo '</li>';
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Most Views Posts', 'newspaper' ),
			'limit'     => 5,
			'show_date' => false,
			'show_view' => false
		);
	...
	?>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_view' ); ?>">
				<?php _e( 'Display post view?', 'newspaper' ); ?>
			</label>
		</p>

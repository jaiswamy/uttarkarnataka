 ...
 * @since      1.0.0
 */
class Newspaper_Posts_1_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-newspaper-posts-1 posts-category-1-widget',
			'description' => __( 'Display posts based on user selected category.', 'newspaper' )
		);

		// Create the widget.
		parent::__construct(
			'newspaper-posts-1',                      // $this->id_base
			__( '&raquo; Category Posts 1', 'newspaper' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
			...
			$query = apply_filters( 'newspaper_widget_posts_1_args', $args );
				...
				<ul>
					...
					<?php while ( $posts->have_posts() ) : $posts->the_post(); ?>
						...
						<?php if ( ++$i == 1 ) { // If first post, show title, excerpt, and image. ?>
							...
							<li class="first clearfix">
								<?php if ( has_post_thumbnail() ) : ?>
									<a href="<?php the_permalink(); ?>"><?php the_post_thumbnail( 'newspaper-widget-thumb', array( 'class' => 'entry-thumb', 'alt' => esc_attr( get_the_title() ) ) ); ?></a>
								<?php endif; ?>
							...
							<?php if ( $instance['show_excerpt'] ) : ?>
								<div class="entry-summary clearfix">
									<?php echo apply_filters( 'newspaper_posts_1', wp_trim_words( get_the_excerpt(), 20 ) ); ?>
								</div><!-- .entry-summary -->
							<?php endif; ?>
							</li>
						...
						<?php } else { // If not the first post, add the entry titles as list items. ?>
							...
							<?php if ( $i == 2 ) // If second post, open the list. ?>
							<li>
					...
					<?php if ( $i > 1 ) // If there is more than one post, close the list after the loop. ?>

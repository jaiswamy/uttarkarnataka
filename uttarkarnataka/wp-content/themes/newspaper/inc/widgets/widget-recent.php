 ...
 * @since      1.0.0
 */
class Newspaper_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-newspaper-recent posts-thumbnail-widget',
			'description' => __( 'Display recent posts with thumbnails.', 'newspaper' )
		);

		// Create the widget.
		parent::__construct(
			'newspaper-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts Thumbnails', 'newspaper' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'newspaper_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'newspaper_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'newspaper_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'newspaper' ),
			'limit'     => 5,
			'show_date' => false
		);
